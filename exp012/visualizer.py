...
class Visualizer(BaseVisualizer):
    ...
    def visualize_result(self, im_input, im_output, im_pred, pred_motion, gt_motion, conflict, file_name='tmp.png', idx=0):
        ...
        conflict = conflict[idx].cpu().data.numpy().squeeze()
        cmap = plt.get_cmap('jet')
        conflict_map = cmap(conflict)[:, :, 0:3]
        x1, y1, x2, y2 = self.get_img_coordinate(3, 3)
        img[y1:y2, x1:x2, :] = conflict_map

        conflict = numpy.minimum(conflict, 1)
        im_diff = numpy.expand_dims(1 - conflict, 2).repeat(3, 2) * im_diff
        x1, y1, x2, y2 = self.get_img_coordinate(2, self.num_frame + 1)
        img[y1:y2, x1:x2, :] = im_diff
