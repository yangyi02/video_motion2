...
class Net(nn.Module):
    ...
    def forward(self, im_input, im_output):
        ...
        out_mask, conflict = construct_mask(m_mask, self.m_kernel, self.m_range)
        im = im_input[:, -self.im_channel:, :, :]
        pred = construct_image(im, out_mask, self.m_kernel, self.m_range)
        return pred, m_mask, conflict
...
def construct_mask(m_mask, m_kernel, m_range):
    ...
    conflict = appear + disappear
    return out_mask, conflict.unsqueeze(1)
