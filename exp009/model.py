...
class Net(nn.Module):
    ...
    def forward(self, im_input, im_output):
        im = torch.cat((im_input, im_output), 1)
        x = self.bn0(self.conv0(im))
        ...
        o_mask = F.sigmoid(self.conv_o(x11))
        ...
        return pred, m_mask, o_mask, appear
...
def construct_mask(m_mask, o_mask, m_kernel, m_range):
    flow_mask = F.conv2d(m_mask, m_kernel, None, 1, m_range, 1, m_kernel.size(0))
    seg = flow_mask.sum(1)
    appear = F.relu(1 - seg)
    occl_mask = F.conv2d(o_mask, m_kernel, None, 1, m_range, 1, m_kernel.size(0))
    out_mask = flow_mask * occl_mask
    seg = out_mask.sum(1)
    out_mask = out_mask / (seg + F.relu(1 - seg)).unsqueeze(1)
    return out_mask, appear.unsqueeze(1)
