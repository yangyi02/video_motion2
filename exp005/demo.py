...
class Demo(BaseDemo):
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            im_pred, m_mask, disappear, appear = self.model(im_input, im_output)
    ...
    def test_gt(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if torch.cuda.is_available():
                ...
                gt_motion = gt_motion.cuda()
            if self.data.name in ['box', 'mnist', 'box_complex']:
                im_pred, m_mask, disappear, appear = self.model_gt(im_input, im_output, gt_motion_label, 'label')
            elif self.data.name in['box2', 'mnist2']:
                im_pred, m_mask, disappear, appear = self.model_gt(im_input, im_output, gt_motion)
            im_diff = (1 - appear) * (im_pred - im_output)
            im_diff = im_diff / (1 - appear).sum(3).sum(2).unsqueeze(2).unsqueeze(3)
            loss = torch.abs(im_diff).sum() * im_diff.size(2) * im_diff.size(3)
